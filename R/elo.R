...
# quick Elo-only win probability function, useful as a baseline to compare
# the trained ML model against
elo_win_prob <- function(elo_a, elo_b, a_is_home = FALSE) {
  adj_a <- elo_a + if (a_is_home) HOME_ADV else 0
  1 / (1 + 10^((elo_b - adj_a) / 400))
}